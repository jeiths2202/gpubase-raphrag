...
export const AdaptiveDocuments = () => {
  ...
  // Single batch polling interval instead of per-file intervals
  const batchPollingRef = useRef<ReturnType<typeof setInterval> | null>(null);
  const pollingTasksRef = useRef<Map<string, { taskId: string; pdfId: string; filename: string; failures: number }>>(new Map());
  const isPollingRef = useRef(false);

  // Constants for polling
  const MAX_FAILURES = 5;
  const POLLING_INTERVAL = 3000; // 3 seconds
  const MAX_CONCURRENT_POLLS = 5; // Maximum concurrent status requests
  ...
  const loadDocuments = async () => {
    try {
      ...
      const uniqueDocs = new Map<string, AdaptiveDocument>();
      for (const doc of docs) {
        const adaptiveDoc: AdaptiveDocument = {
          pdf_id: doc.pdf_id,
        ...
        // Keep the first occurrence (or could keep latest by created_at)
        if (!uniqueDocs.has(doc.pdf_id)) {
          uniqueDocs.set(doc.pdf_id, adaptiveDoc);
        }
      }
  ...
  const loadPendingFiles = async () => {
    try {
      ...
      const uniqueFiles = new Map<string, PendingUploadItem>();
      for (const file of response.files) {
        if (!uniqueFiles.has(file.filename)) {
          uniqueFiles.set(file.filename, file);
        }
      }
      setPendingFiles(Array.from(uniqueFiles.values()));
    } catch (err) {
  ...
  const handleStartEmbedding = async () => {
    ...
    try {
      ...
      // Add to polling queue for each processing file
      for (const item of result.results) {
        if (item.status === 'processing' && item.task_id && item.pdf_id) {
          addPollingTask(item.task_id, item.pdf_id, item.filename);
        }
      }
  ...
  // Add a task to the batch polling queue
  const addPollingTask = (taskId: string, pdfId: string, filename: string) => {
    const pollKey = `embed_${pdfId}`;
    pollingTasksRef.current.set(pollKey, { taskId, pdfId, filename, failures: 0 });

    // Start batch polling if not already running
    if (!batchPollingRef.current && !isPollingRef.current) {
      startBatchPolling();
    }
  };

  // Batch polling: poll all tasks in controlled batches
  const startBatchPolling = () => {
    if (isPollingRef.current) return;
    isPollingRef.current = true;

    const pollBatch = async () => {
      const tasks = Array.from(pollingTasksRef.current.entries());
      if (tasks.length === 0) {
        // No more tasks to poll, stop the interval
        if (batchPollingRef.current) {
          clearInterval(batchPollingRef.current);
          batchPollingRef.current = null;
        }
      ...
      // Poll only MAX_CONCURRENT_POLLS at a time
      const batch = tasks.slice(0, MAX_CONCURRENT_POLLS);

      // Process batch sequentially to avoid overwhelming the server
      for (const [pollKey, task] of batch) {
        try {
          const status = await adaptiveDocumentsApi.getStatus(task.taskId);
          ...
          if (status.status === 'completed' || status.status === 'failed') {
            ...
            pollingTasksRef.current.delete(pollKey);

            // If completed, add to documents list (avoiding duplicates)
            if (status.status === 'completed') {
              setDocuments(prev => {
                // Check if already exists
                if (prev.some(d => d.pdf_id === task.pdfId)) {
                  return prev;
                }
                return [{
                  pdf_id: task.pdfId,
          ...
          task.failures += 1;
          console.warn(`Embedding polling error for ${task.filename} (${task.failures}/${MAX_FAILURES}):`, err);

          // Stop polling this task after too many consecutive failures
          if (task.failures >= MAX_FAILURES) {
            console.error(`Stopping polling for ${task.filename} after ${MAX_FAILURES} failures`);
            pollingTasksRef.current.delete(pollKey);
    ...
    // Start the batch polling interval
    batchPollingRef.current = setInterval(pollBatch, POLLING_INTERVAL);
    // Run immediately
    pollBatch();
  };
