...
# -*- coding: utf-8 -*-
"""
PDF Extractor 검증 프로그램
===========================
manual_mvs_7.1 디렉토리 내 모든 PDF 파일에 대해
openframe_qlora_full_extractor.py의 PDF 파싱 모드를 검증합니다.
"""
...
# extractor 모듈 import
from openframe_qlora_full_extractor import (
    OpenFrameManualParser,
    QLoRADataGenerator,
    TrainingExample
)


class PDFExtractorValidator:
    """PDF Extractor 검증 클래스"""

    def __init__(self, manual_dir: str, output_dir: str = "validation_output"):
    ...
    def validate_single_pdf(self, pdf_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            generator = QLoRADataGenerator(parser)
            training_data = generator.generate_training_data(language="ja")
            ...
            result["qa_pairs_count"] = len(qa_pairs)
            ...
            output_filename = pdf_path.stem + "_training_data.jsonl"
    ...
    def validate_all(self) -> List[Dict[str, Any]]:
        ...
        print(f"\n{'#'*60}")
        print(f"# OpenFrame MVS 7.1 PDF Extractor 검증")
    ...
    def print_summary(self):
        ...
        for r in self.results:
            if r["status"] == "success":
                print(f"  ✓ {r['filename']}")
                print(f"    섹션: {r['sections_count']}, 학습데이터: {r['total_examples']}")
...
def main():
    ...
    validator = PDFExtractorValidator(manual_dir)
