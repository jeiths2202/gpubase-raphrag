...
async def test_vllm_connection(
    ...
    model = os.getenv("LEARNING_LLM_MODEL", "/opt/models/qwen3-32b")
...
@router.get(
    "/monitor/gpu",
    summary="GPU 상태 모니터링",
    description="원격 서버의 GPU 상태를 조회합니다. show_all=true면 모든 GPU를 표시합니다.",
)
async def get_gpu_status(
    ...
    """GPU 상태 조회 (원격 서버 지원)"""
    from app.api.services.remote_gpu_service import get_remote_gpu_service

    try:
        gpu_service = get_remote_gpu_service()
        status = await gpu_service.get_gpu_status(show_all=show_all)
...
@router.get(
    "/monitor/gpu/{gpu_index}",
    summary="GPU 상세 정보 조회",
    description="특정 GPU의 상세 정보와 실행 중인 프로세스를 조회합니다.",
)
async def get_gpu_detail(
    gpu_index: int = Path(..., description="GPU 인덱스 (0-7)"),
    user=Depends(get_current_user),
):
    """GPU 상세 정보 조회 (원격 서버 지원)"""
    from app.api.services.remote_gpu_service import get_remote_gpu_service

    try:
        gpu_service = get_remote_gpu_service()
        status = await gpu_service.get_gpu_detail(gpu_index)
        return status.to_dict()
    except Exception as e:
