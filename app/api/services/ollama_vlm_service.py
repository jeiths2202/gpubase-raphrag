...
class OllamaVLMService:
    """
    Ollama-based VLM service for multimodal RAG.

    Uses bakllava model for:
    ...
    def __init__(
        ...
        Args:
            ...
            model: VLM model name (bakllava, llava, etc.)
            timeout: Request timeout in seconds
        """
        self.base_url = base_url.rstrip('/')
        self.model = model
        self.timeout = timeout
        # bakllava generates 4096-dimensional embeddings
...
4. How this might relate to a document context"""
    ...
    async def _generate_with_image(
        ...
        """
        Call Ollama generate API with an image.
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if response.status == 200:
                        data = await response.json()
                        # Check if bakllava model is available
...
def get_ollama_vlm_service() -> OllamaVLMService:
    """Get singleton Ollama VLM service instance."""
    global _vlm_service_instance
    if _vlm_service_instance is None:
        _vlm_service_instance = OllamaVLMService()
