...
class AgentOrchestrator:
    ...
    async def execute(
        ...
        # Searches file-based summaries for error codes and glossary terms
        # =====================================================================
        try:
            from ..services.summary_search_service import get_summary_search_service
            summary_service = get_summary_search_service()

            # Get error context (for error codes like -5212)
            error_context = await summary_service.get_error_context_for_agent(request.task)
            if error_context:
                context.metadata['summary_error_context'] = error_context
                logger.info(f"[Orchestrator] Summary error context found: {len(error_context)} chars")

            # Get term context (for technical terms like TJES, TACF)
            term_context = await summary_service.get_term_context_for_agent(request.task)
            if term_context:
                context.metadata['summary_term_context'] = term_context
                logger.info(f"[Orchestrator] Summary term context found: {len(term_context)} chars")

            # Get command context (for commands like tjesinit, oscboot)
            cmd_context = await summary_service.get_command_context_for_agent(request.task)
            if cmd_context:
                context.metadata['summary_command_context'] = cmd_context
                logger.info(f"[Orchestrator] Summary command context found: {len(cmd_context)} chars")

            # Get API context (for API functions like tcfh_stow, tfcd_read)
            api_context = await summary_service.get_api_context_for_agent(request.task)
            if api_context:
                context.metadata['summary_api_context'] = api_context
                logger.info(f"[Orchestrator] Summary API context found: {len(api_context)} chars")

            # Get product context (for product overview requests)
            product_context = await summary_service.get_product_context_for_agent(request.task)
            if product_context:
                context.metadata['summary_product_context'] = product_context
                logger.info(f"[Orchestrator] Summary product context found: {len(product_context)} chars")
        except Exception as e:
            logger.warning(f"[Orchestrator] Summary search failed (non-blocking): {e}")
    ...
    async def stream(
        ...
        try:
            ...
            if error_context:
                context.metadata['summary_error_context'] = error_context
                logger.info(f"[Orchestrator.stream] Summary error context found: {len(error_context)} chars")

            # Get term context (for technical terms like TJES, TACF)
            term_context = await summary_service.get_term_context_for_agent(request.task)
            if term_context:
                context.metadata['summary_term_context'] = term_context
                logger.info(f"[Orchestrator.stream] Summary term context found: {len(term_context)} chars")

            # Get command context (for commands like tjesinit, oscboot)
            cmd_context = await summary_service.get_command_context_for_agent(request.task)
            if cmd_context:
                context.metadata['summary_command_context'] = cmd_context
                logger.info(f"[Orchestrator.stream] Summary command context found: {len(cmd_context)} chars")

            # Get API context (for API functions like tcfh_stow, tfcd_read)
            api_context = await summary_service.get_api_context_for_agent(request.task)
            if api_context:
                context.metadata['summary_api_context'] = api_context
                logger.info(f"[Orchestrator.stream] Summary API context found: {len(api_context)} chars")

            # Get product context (for product overview requests)
            product_context = await summary_service.get_product_context_for_agent(request.task)
            if product_context:
                context.metadata['summary_product_context'] = product_context
                logger.info(f"[Orchestrator.stream] Summary product context found: {len(product_context)} chars")
        except Exception as e:
            logger.warning(f"[Orchestrator.stream] Summary search failed (non-blocking): {e}")
